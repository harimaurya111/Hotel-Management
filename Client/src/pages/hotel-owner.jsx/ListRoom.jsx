...
//ToggleAvailibity of the room
const toggleRoomAvailability = async (roomId) => {
  const {data} = await axios.post("/api/rooms/toggle-availabillity",{roomId} , {headers : {Authorization : `Bearer ${await getToken()}` }})
  if(data.success){
              ...
              <tr key={index}>
                ...
                <td className="py-3 px-4 border-t border-gray-300 text-sm text-red-500 text-center">
                  <label className="relative inline-flex items-center cursor-pointer gap-3 text-gray-900">
                    <input onChange={()=>toggleRoomAvailability(room._id)} type="checkbox" className="sr-only peer"/>
