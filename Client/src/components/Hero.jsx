...
const Hero = () => {
  ...
  const onSearch = async (e) => {
    ...
    // Navigate to filtered rooms page
    navigate(`/rooms?destination=${trimmedDestination}`);

    // Save to DB (recent search)
    try {
      await axios.post(
      ...
      // Add to local context state (max 3 items, avoid duplicates)
      setSearchCities((previous) => {
        const updated = [...previous];
        if (!updated.includes(trimmedDestination)) {
          updated.push(trimmedDestination);
  ...
  return (
    <div className='flex flex-col items-start justify-center px-6 md:px-16 lg:px-24 xl:px-32 text-white bg-[url("/src/assets/heroImage.png")] bg-no-repeat bg-cover bg-center h-screen'>
      ...
      <p className='max-w-[30rem] mt-2 text-sm md:text-base'>
